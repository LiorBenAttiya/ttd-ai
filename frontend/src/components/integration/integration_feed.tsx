          ...
          {query && (
            <button onClick={() => setQuery('')}
                    style={{ position: 'absolute', right: 12, background: 'none', border: 'none',
                             color: '#475569', cursor: 'pointer', fontSize: 14, lineHeight: 1 }}>✕</button>
          )}
        </div>
        ...
        {query.trim() && (
          <div style={{ marginTop: 6, borderRadius: 12, overflow: 'hidden',
            ...
            <div style={{ padding: '8px 14px 6px', borderBottom: '1px solid #E2E8F0',
              ...
              {searching && <span style={{ fontSize: 10, color: '#475569' }}>⏳</span>}
            </div>

            {!searching && results.length === 0 && (
              <div style={{ padding: '20px 14px', textAlign: 'center' }}>
                <div style={{ fontSize: 20, marginBottom: 6 }}>🔍</div>
                <div style={{ fontSize: 12, color: '#475569' }}>No results for <strong style={{ color: '#64748B' }}>"{query}"</strong></div>
                <div style={{ fontSize: 10, color: '#334155', marginTop: 4 }}>Try a contact name, task keyword, or email subject</div>
              </div>
            )}

            {!searching && results.map((r, i) => {
              const dotColor = r.type === 'task' && r.priority
                ? (PRIORITY_COLOR[r.priority as 1|2|3] ?? '#818CF8')
                : TYPE_COLOR[r.type] ?? '#818CF8'
              return (
                <div key={r.id}
                  ...
                  {/* Type icon */}
                  <span style={{ fontSize: 14, flexShrink: 0, lineHeight: 1 }}>{r.icon}</span>
                  ...
                  <span style={{
                    ...
                    background: `${TYPE_COLOR[r.type] ?? '#818CF8'}18`,
                    color: TYPE_COLOR[r.type] ?? '#818CF8',
